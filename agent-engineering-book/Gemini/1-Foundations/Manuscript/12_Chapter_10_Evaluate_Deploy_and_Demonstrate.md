...
This chapter turns a working application into an evaluated, deployable baseline — proof that the single-account system is trustworthy on its own terms, before Chapter 11 wraps it for unattended, multi-account operation. The reader learns to measure final-answer quality, contract compliance, tool behavior, workflow trajectories, safety boundaries, latency, and operational readiness.
...
Create the evaluated single-account baseline: an application deployed to Cloud Run, with basic observability and a repeatable demonstration — trustworthy for one account at a time, before Chapter 11 makes it safe to run unattended.
...
Chapter 1 established Cloud Run as Book 1's canonical deployment target, and this chapter deploys to it: a transparent, containerized application model that matches the reference architecture built in §1.12.

A supported **managed agent runtime** remains a viable alternative for tighter lifecycle integration, if a reader's environment or learning objective calls for it — but it is a deliberate substitution for the book's own path, not an equally-weighted default.
...
This chapter's evaluated baseline should not be considered trustworthy — let alone wrapped in Chapter 11's unattended loop — unless:
...
## Hands-on lab: Establish the Evaluated Baseline
