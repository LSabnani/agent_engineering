...
These four values replace the casual `QUALIFY` / `DO_NOT_QUALIFY` / `NEEDS_RESEARCH` prose recommendation Chapter 4 built. That earlier vocabulary was deliberately informal — there was no contract yet to validate it. From this chapter forward, only the enumeration above is the qualification status; nothing downstream should ever see `QUALIFY` or `DO_NOT_QUALIFY` again.
