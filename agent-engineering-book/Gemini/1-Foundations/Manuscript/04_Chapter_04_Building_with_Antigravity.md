# Chapter 4: Building with Google Antigravity

*Creating the Engineering Harness for WidgetWare SDR*

## Chapter purpose

Chapter 3 defined what WidgetWare SDR is allowed to do, where it fits in the sales process, which decisions belong to models, software, and people, and what the completed Google Cloud architecture will contain. This chapter turns that intent into an inspectable development workspace.
...
Chapter 4 does not build the WidgetWare business agent. It builds the harness in which that agent will be created safely over the remaining chapters.
...
Chapter 3 presented the completed Book 1 architecture. That diagram was intentionally ambitious: Cloud Run, Gemini, ADK agents, tools, MCP integrations, persistent state, observability, approval, and a bounded batch loop.
...
<div class="figure-page landscape">
  <img src="figures/chapter4/the-agent-engineering-harness.png" alt="Figure 4.1 — The Agent Engineering Harness" />
</div>
...
<div class="figure-page landscape">
  <img src="figures/chapter4/the-antigravity-engineering-cycle.png" alt="Figure 4.2 — The Antigravity Engineering Cycle" />
</div>
...
It does not need Chapter 10’s future MCP research contract.
...
<div class="figure-page landscape">
  <img src="figures/chapter4/widgetware-repository-architecture.png" alt="Figure 4.3 — WidgetWare Repository Architecture" />
</div>
...
The `config/` directory will hold product, ICP, and policy files beginning in Chapter 5. In Chapter 4, a `README.md` can define what belongs there and prohibit credentials.
...
<div class="figure-page landscape">
  <img src="figures/chapter4/anatomy-of-a-specification-driven-task.png" alt="Figure 4.4 — Anatomy of a Specification-Driven Task" />
</div>
...
<div class="figure-page landscape">
  <img src="figures/chapter4/trust-permissions-and-change-acceptance-gates.png" alt="Figure 4.5 — Trust, Permissions, and Change-Acceptance Gates" />
</div>
...
Chapter 4 requires no production secrets. If a tool asks for one, treat that as a design warning. Use placeholders in `.env.example`, local ignored files for later development credentials, and managed secret storage in deployed environments.
...
At the end of Chapter 4, these variables may remain unused. Documenting them early is acceptable if the README clearly distinguishes future setup from current requirements.
...
The checkpoint gives every later chapter a known starting point. A learner who joins at Chapter 5 should be able to clone or check out this state and reproduce the same verification result.
...
Create the directories and baseline files shown in Section 2.5. Copy the Chapter 3 business brief, architecture, authority boundary, and acceptance criteria into `docs/`.
...
Ask Antigravity to compare the repository against `SPEC.md` and the Chapter 4 acceptance criteria. The report must distinguish:
...
Run the baseline command from a clean state. Commit the accepted Chapter 4 files and record the checkpoint identifier.
...
Chapter 5 defines how Gemini will receive instructions and WidgetWare business knowledge. The goal is not to create one increasingly large prompt. It is to build an explicit context architecture that separates stable policy, business configuration, task input, workflow state, and retrieved evidence.

The Chapter 4 harness makes that work possible. The repository now has a place for configuration, a specification that defines evidence and authority, tests that can protect required behavior, and a development cycle that makes each change reviewable.
...
6. **Practice rollback.** Make a harmless experimental change on a task branch or worktree, run the checks, inspect the diff, and revert to the Chapter 4 checkpoint. Record the commands and evidence proving the baseline was restored.

7. **Predict repository growth.** Using the repository architecture in Section 2.5, predict where Chapters 5 through 8 will place business configuration, context assembly, agent definitions, Skills, contracts, and tests. Revisit the prediction after Chapter 8.
