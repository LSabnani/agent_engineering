# Chapter 2: Building with Google Antigravity

*Creating the Engineering Harness for WidgetWare SDR*

## Chapter purpose

Chapter 1 defined what WidgetWare SDR is allowed to do, where it fits in the sales process, which decisions belong to models, software, and people, and what the completed Google Cloud architecture will contain. This chapter turns that intent into an inspectable development workspace.
...
Chapter 2 does not build the WidgetWare business agent. It builds the harness in which that agent will be created safely over the remaining chapters.
...
## From destination architecture to a working repository

Chapter 1 presented the completed Book 1 architecture. That diagram was intentionally ambitious: Cloud Run, Gemini, ADK agents, tools, MCP integrations, persistent state, observability, approval, and a bounded batch loop.

A common mistake is to treat that architecture as a request to generate everything at once. A coding agent may respond with hundreds of files, several frameworks, inferred requirements, placeholder security, and a project that appears complete but is difficult to verify. The result may be impressive as a demonstration and weak as an engineering baseline.
...
The first objective is not to make WidgetWare smart. It is to make WidgetWare buildable.
...
### Figure 2.1 — The Agent Engineering Harness

<div class="figure-page landscape">
  <img src="figures/chapter2/the-agent-engineering-harness.png" alt="Figure 2.1 — The Agent Engineering Harness" />
</div>
...
### Figure 2.2 — The Antigravity Engineering Cycle

<div class="figure-page landscape">
  <img src="figures/chapter2/the-antigravity-engineering-cycle.png" alt="Figure 2.2 — The Antigravity Engineering Cycle" />
</div>

## 2.3 The Antigravity engineering cycle
...
It does not need Chapter 8’s future MCP research contract.
...
## 2.4 Projects, workspaces, and isolated experiments
...
## 2.5 Repository architecture

Repository structure is an architectural control. It tells humans and agents where a type of truth belongs.

### Figure 2.3 — WidgetWare Repository Architecture

<div class="figure-page landscape">
  <img src="figures/chapter2/widgetware-repository-architecture.png" alt="Figure 2.3 — WidgetWare Repository Architecture" />
</div>
...
The `config/` directory will hold product, ICP, and policy files beginning in Chapter 3. In Chapter 2, a `README.md` can define what belongs there and prohibit credentials.
...
## 2.6 README, SPEC, and architecture decisions
...
## 2.7 Repository instructions, rules, and workflows
...
## 2.8 Anatomy of a specification-driven task

A specification-driven task converts one approved objective into a bounded unit of work.

### Figure 2.4 — Anatomy of a Specification-Driven Task

<div class="figure-page landscape">
  <img src="figures/chapter2/anatomy-of-a-specification-driven-task.png" alt="Figure 2.4 — Anatomy of a Specification-Driven Task" />
</div>
...
The Chapter 2 health-check task can be written as follows:
...
## 2.10 Trust, permissions, and change-acceptance gates
...
### Figure 2.5 — Trust, Permissions, and Change-Acceptance Gates

<div class="figure-page landscape">
  <img src="figures/chapter2/trust-permissions-and-change-acceptance-gates.png" alt="Figure 2.5 — Trust, Permissions, and Change-Acceptance Gates" />
</div>
...
A practical policy for Chapter 2 is:
...
| Read credentials or local secret files | Restricted | Not required for Chapter 2 |
...
Chapter 2 requires no production secrets. If a tool asks for one, treat that as a design warning. Use placeholders in `.env.example`, local ignored files for later development credentials, and managed secret storage in deployed environments.

Permission is not merely a convenience setting. It is part of the engineering control system.

## 2.11 Reviewing changes and evidence
...
## 2.12 Environment, dependency, and secret management
...
At the end of Chapter 2, these variables may remain unused. Documenting them early is acceptable if the README clearly distinguishes future setup from current requirements.

## 2.13 Baseline quality gates
...
## 2.14 Version control, checkpoints, and rollback
...
A useful Chapter 2 checkpoint might be:

> `chapter-02: establish reproducible workspace and health-check baseline`

The checkpoint gives every later chapter a known starting point. A learner who joins at Chapter 3 should be able to clone or check out this state and reproduce the same verification result.
...
Create the directories and baseline files shown in Section 2.5. Copy the Chapter 1 business brief, architecture, authority boundary, and acceptance criteria into `docs/`.
...
Use the task contract from Section 2.8. Ask for a plan before implementation. Review the plan and remove any unjustified dependency or out-of-scope change.
...
Ask Antigravity to compare the repository against `SPEC.md` and the Chapter 2 acceptance criteria. The report must distinguish:

- requirements satisfied now;
- capabilities intentionally deferred;
- gaps that block Chapter 3; and
...
Run the baseline command from a clean state. Commit the accepted Chapter 2 files and record the checkpoint identifier.
...
- an accepted Chapter 2 Git checkpoint; and
...
Chapter 3 defines how Gemini will receive instructions and WidgetWare business knowledge. The goal is not to create one increasingly large prompt. It is to build an explicit context architecture that separates stable policy, business configuration, task input, workflow state, and retrieved evidence.

The Chapter 2 harness makes that work possible. The repository now has a place for configuration, a specification that defines evidence and authority, tests that can protect required behavior, and a development cycle that makes each change reviewable.

## Exercises

1. **Compare unrestricted and bounded work.** On a throwaway branch, give Antigravity the instruction “build the entire application.” Do not accept the changes. Record the plan, number of files touched, assumptions made, and review effort. Then give it the bounded health-check task from Section 2.8. Compare inspectability rather than code volume.
...
6. **Practice rollback.** Make a harmless experimental change on a task branch or worktree, run the checks, inspect the diff, and revert to the Chapter 2 checkpoint. Record the commands and evidence proving the baseline was restored.

7. **Predict repository growth.** Using the repository architecture in Section 2.5, predict where Chapters 3 through 6 will place business configuration, context assembly, agent definitions, Skills, contracts, and tests. Revisit the prediction after Chapter 6.
