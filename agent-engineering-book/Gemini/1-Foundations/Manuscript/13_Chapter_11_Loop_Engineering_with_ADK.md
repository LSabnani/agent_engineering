...
- use ADK's `LoopAgent`, `max_iterations`, and escalation mechanism (the conventional `exit_loop` application function) correctly and understand what they do and do not guarantee;
...
This is the same thirteen-state machine Chapter 9.3 already built for one account, including `DISQUALIFIED` for a clean no-fit decision and the loop-backs to `RESEARCHING` and `DRAFT_READY`. The loop does not replace it — it adds two more states that only make sense once accounts are processed unattended, in a batch, rather than one at a time on request:
...
`exit_loop` is not an ADK-provided built-in tool — it is a small, ordinary application function WidgetWare defines and attaches to the relevant sub-agent, whose entire body sets `tool_context.actions.escalate = True`. The escalation *mechanism* is what ADK actually supplies: `EventActions.escalate` is the signal a `LoopAgent` checks after every sub-agent turn to decide whether to stop iterating. `exit_loop` is just the conventional name for the function that trips that signal — the loop can be stopped early without the `LoopAgent` itself needing to inspect every field of every result to figure that out. What ADK's mechanism does not do on its own is distinguish *why* the loop is stopping: that interpretation is WidgetWare's own code. The same `escalate` signal, paired with the account's session state, is what the loop's control-flow code reads to decide whether this is a clean STOP (budget reached, queue empty) or an ESCALATE (route to `NEEDS_HUMAN_REVIEW`) — ADK hands back control; WidgetWare's decision logic assigns the reason.
