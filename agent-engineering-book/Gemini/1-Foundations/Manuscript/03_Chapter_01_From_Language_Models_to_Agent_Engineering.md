...
This chapter establishes the conceptual, business, and architectural foundation for Book 1. The reader learns why a language model is a capability rather than a complete system, when agent behavior is justified, how autonomy should be bounded, where Sales Development Representatives fit within a formal business-to-business sales process, and what the completed WidgetWare SDR system will look like on Google Cloud.

Chapter 1 does not implement an agent. It defines the problem, the authority boundary, the expected artifacts, the target architecture, and the evidence that will later demonstrate success. Those decisions must exist before model prompts, tools, workflows, or deployment code.
...
- explain the purpose of each major component in the Book 1 Google Cloud reference architecture; and
...
Create the initial product and architecture brief for the WidgetWare SDR system:
...
## 1.2 Models, assistants, workflows, agents, and agentic systems
...
## 1.4 Autonomy is an engineering decision
...
## 1.5 The Seven Steps to Agent Engineering

Book 1 uses one vendor-independent framework across Gemini, ADK, Antigravity, tools, Skills, workflows, loops, and Google Cloud services. The technologies are implementations; the steps describe the engineering work.

### Figure 1.1 — The Seven Steps to Agent Engineering

<div class="figure-page landscape">
  <img src="figures/chapter1/the-seven-steps-to-agent-engineering.png" alt="Figure 1.1 — The Seven Steps to Agent Engineering" />
</div>
...
The numbered steps are not a rigid chapter order. Chapter 2 builds the development harness before Chapter 3 completes context because the reader needs a disciplined workspace in which to place the business rules. The framework describes engineering concerns and dependencies, not a mandatory waterfall.

Evaluation and governance appear as Step 7 because they provide the evidence required to release and improve the system. They are also cross-cutting disciplines. We define acceptance criteria in Chapter 1, write tests while building the harness, preserve evidence during research, enforce approval during orchestration, verify progress inside loops, and perform integrated evaluation before release.
...
## 1.6 Understanding B2B sales and the SDR role
...
## 1.7 The formal B2B sales process
...
### Figure 1.2 — End-to-end B2B sales lifecycle

<div class="figure-page landscape">
  <img src="figures/chapter1/end-to-end-b2b-sales-lifecycle.png" alt="Figure 1.2 — End-to-End B2B Sales Cycle" />
</div>
...
## 1.8 The SDR operating process

The SDR portion of the sales lifecycle can itself be represented as a controlled workflow.

### Figure 1.3 — Human SDR operating process

<div class="figure-page landscape">
  <img src="figures/chapter1/the-sdr-operating-process.png" alt="Figure 1.3 — The SDR Operating Process" />
</div>
...
## 1.9 Why SDR is an effective Agent Engineering case
...
## 1.10 The WidgetWare business brief
...
| Size | Mid-market to large enterprise; thresholds become configurable in Chapter 3 |
...
## 1.11 The Book 1 business workflow and artifacts

The completed Book 1 system transforms a supplied target account into an inspectable approval package.

### Figure 1.4 — WidgetWare information and artifact flow

<div class="figure-page landscape">
  <img src="figures/chapter1/widgetware-sdr-information-and-artifact-flow.png" alt="Figure 1.4 — WidgetWare SDR Information & Artifact Flow" />
</div>
...
## 1.12 Book 1 Google Cloud reference architecture

The following diagram shows the **Book 1 target architecture**, not the Chapter 1 implementation. Readers build it incrementally across the remaining chapters.

The design adapts Google Cloud guidance for ADK-based agent systems on Cloud Run, including standardized tool access through Model Context Protocol integrations and observability through Cloud Logging, Monitoring, and Trace.[^gcp-reference-architecture] Cloud Run is Book 1's canonical deployment target — the architecture the rest of this book builds toward and the one Chapter 10 deploys. A managed agent runtime remains a viable alternative for a reader who wants to deviate, but it is a deliberate substitution, not an equally-weighted default.

### Figure 1.5 — WidgetWare SDR Book 1 target architecture

<div class="figure-page landscape">
  <img src="figures/chapter1/widgetware-sdr-book-1-reference-architecture.png" alt="Figure 1.5 — WidgetWare SDR Book 1 Reference Architecture (Google Cloud)" />
</div>

The components have clear responsibilities:

- **Google Antigravity IDE** is the specification-driven development harness. Its agent can work across the editor, terminal, and browser while exposing artifacts for review.[^antigravity]
- **Cloud Build and Artifact Registry** produce and store a versioned container image.
- **Cloud Run services** host the interactive API and approval interface.
- **A Cloud Run job** hosts the finite Chapter 11 batch process. Cloud Scheduler can invoke it using an authorized service account.[^scheduled-jobs]
- **ADK** supplies agents, tools, sessions, state, and orchestration; Google Cloud describes it as a modular framework for simple through multi-agent workflows.[^adk-components]
- **Gemini on Vertex AI** supplies interpretation, synthesis, comparison, and drafting.
- **Typed tools and approved MCP services** provide controlled access to internal and external information.
- **Cloud SQL** stores operational state; **Cloud Storage** holds larger evidence and evaluation artifacts.
- **IAM and Secret Manager** keep identity, permission, and credentials outside model reasoning.
- **Google Cloud Observability** traces execution without requiring the runtime container to retain state.
- **The human approval gate** ends Book 1. Approval is recorded, but no send tool exists.

### How the architecture grows across the book

| Chapter | Architecture increment |
|---|---|
| 1 | Business boundary, Seven Steps, target workflow, destination architecture |
| 2 | Antigravity workspace, repository conventions, tests, development harness |
| 3 | Product context, ICP, policies, model configuration, evidence rules |
| 4 | First ADK agent connected to Gemini |
| 5 | Reusable qualification and evidence-classification Skills |
| 6 | Typed artifacts and agent contracts |
| 7 | Controlled internal tools |
| 8 | MCP research and evidence ledger |
| 9 | Multi-agent workflow, state transitions, human approval |
| 10 | Evaluation, observability, containerization, Cloud Run deployment |
| 11 | Persistent queue, checkpoints, budgets, stopping decisions, batch execution |

The architecture is a destination map. Every later component must earn its place by satisfying a business or control requirement.

## 1.13 System boundaries and human control
...
## 1.14 Define success before implementation
...
8. Mark every external system and trust boundary in the reference architecture.
...
- Does the reference architecture externalize state from the runtime?
- Does each proposed Google Cloud component have a clear responsibility?
...
The repository now contains a clear business definition, formal sales-process position, autonomy boundary, destination architecture, and measurable success criteria for WidgetWare SDR.
...
- how the completed system will run on Google Cloud; and
- what evidence will demonstrate success.

## Bridge to Chapter 2

The next chapter builds the engineering harness in Google Antigravity. Before giving Gemini or ADK more capability, we create the workspace, repository conventions, specifications, permissions, test discipline, and review cycle that will keep development inspectable.

The target architecture is now visible. Chapter 2 creates the environment in which it can be built safely, one bounded increment at a time.
