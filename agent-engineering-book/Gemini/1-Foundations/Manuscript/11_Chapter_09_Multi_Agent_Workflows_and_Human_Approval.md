...
Three of these thirteen states exist specifically because a `QualificationResult` (Chapter 6) or an approval decision can produce more than one kind of outcome, and the workflow needs a distinct, nameable state for each — never an implicit fallthrough:

| Trigger | Resulting state | What happens next |
|---|---|---|
| `QualificationResult.status == QUALIFIED` | → `REVIEW_REQUIRED` | proceeds to evidence review |
| `QualificationResult.status == NOT_QUALIFIED` | → `DISQUALIFIED` (terminal) | the account settles cleanly — not an error, not an approval outcome, just a fit decision the evidence supports |
| `QualificationResult.status == NEEDS_RESEARCH` | → `NEEDS_RESEARCH` → back to `RESEARCHING` | the workflow loops back for more evidence rather than guessing |
| `QualificationResult.status == BLOCKED` | → `BLOCKED` (terminal) | a parsing or policy failure, not a business decision |
| Approval decision: approve | → `APPROVED` (terminal) | — |
| Approval decision: reject | → `REJECTED` (terminal) | — |
| Approval decision: revise | → `REVISION_REQUESTED` → back to `DRAFT_READY` | the reviewer's requested change is incorporated and a new draft is produced |

`DISQUALIFIED` and `BLOCKED` look similar from the outside — both stop an account short of approval — but they mean opposite things. `DISQUALIFIED` is a successful outcome: the system did its job and the evidence says no. `BLOCKED` is a failure of the pipeline itself: something could not be parsed, validated, or resolved, and no business decision was actually reached.
...
- approve, reject, or revise controls, mapping directly to `APPROVED`, `REJECTED`, or `REVISION_REQUESTED`.
...
- scenario tests for success, clean disqualification, insufficient evidence (loops back to research), source conflict, malformed output, rejected approval, and a requested revision (loops back to drafting).
...
Chapter 10 asks whether this single-account system is actually good enough on its own terms — not yet the final Book 1 release, but the evaluated baseline Chapter 11 needs before it can be trusted to run unattended. We will build evaluation datasets, inspect trajectories, test failure conditions, add observability, and deploy the integrated application.
...
2. §9.3 requires the state machine to exist before the agent prompts. Draw, on paper, the thirteen states from §9.3, then trace by hand what happens to an account that fails research twice before succeeding on a third attempt — which states does it pass through, and how many times does it revisit `RESEARCHING`?
