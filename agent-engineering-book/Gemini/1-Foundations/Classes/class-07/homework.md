...
The provided test suite includes a case where a `QualificationResult` is constructed with `status=QUALIFIED` and an `evidence_refs` entry — but that entry doesn't actually correspond to any fact a tool returned; it was invented. Nothing in Class 6's contract validation catches that, because the contract only checks that the *field is non-empty*, not that its contents are *real*. Write a test that would catch a fabricated evidence reference, and explain in one sentence why the contract layer alone (Class 6) can't fully solve this — what layer would need to?
