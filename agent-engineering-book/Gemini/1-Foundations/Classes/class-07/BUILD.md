...
2. Write `calculate_fit_score()` yourself, by hand, in `tools/fit_score.py` — it's pure arithmetic, and Book 1 §9 is explicit that this kind of calculation belongs outside model reasoning. Do not expose it to the agent as a tool; call it from application code only.
