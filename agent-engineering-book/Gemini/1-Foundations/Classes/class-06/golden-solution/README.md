...
This checkpoint (Class 6 / `golden-solutions/class-06/`) replaces the agent's free-form prose output with a validated `QualificationResult` contract. The agent itself, its Skill, and its model call are unchanged from Class 5 — this chapter is a standalone validation layer, not a rewiring of the agent.
...
│   ├── skills.py
│   ├── app.py
│   └── agents/qualification_agent.py   # unchanged from Class 5 — still no tools
├── tests/
│   ├── unit/
│   ├── contracts/
...
print(result.status)   # BLOCKED — QUALIFIED without evidence_refs is an invariant violation, not a valid result
...
- [ ] The agent (`qualification_agent.py`) is byte-for-byte unchanged from Class 5 — this chapter adds a validation layer, it doesn't touch the agent.
- [ ] The agent still has no tools attached.

## Starting Class 7

1. Start from this checkpoint. Class 7 gives the agent its first real tools — three narrow, read-only functions for account, product, and ICP data — and updates its instruction to use them instead of assuming facts.
2. The `QualificationResult` and `EvidenceItem` contracts don't change structurally in Class 7, but `EvidenceItem` starts getting real exercise once tool-retrieved facts need evidence identifiers.
...
- [ ] Classes 7–11 — see `../../00_Course_Framework.md`
