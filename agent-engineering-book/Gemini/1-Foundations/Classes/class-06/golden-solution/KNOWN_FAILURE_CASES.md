...
`QualificationResult` and `EvidenceItem` are fully defined and tested in isolation — schema validation and the four business invariants all pass. But `qualification_agent.py` is completely unchanged from Class 5: it still returns free-form prose, and nothing in this checkpoint calls `parse_qualification_result()` on a real agent response. Per Book 1 §6's own Hands-on Lab, this is the intended scope for this chapter (contracts as a standalone deliverable) — the wiring that actually makes the agent's output conform happens implicitly as later chapters build on top of this contract, not as an explicit step in this one.
