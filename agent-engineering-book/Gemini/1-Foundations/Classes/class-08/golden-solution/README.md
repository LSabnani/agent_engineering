...
This checkpoint (Class 8 / `golden-solutions/class-08/`) adds the agent's first capability to look outside WidgetWare's own data: a research pipeline that gathers external evidence, normalizes it into the `EvidenceItem` contract, detects conflicts between sources, and never lets a claim exist without a citation — treating everything retrieved as untrusted content throughout.
...
print(brief.conflicts[0])     # the employee-count conflict, both values, both sources — never silently resolved
```
...
1. Start from this checkpoint. Class 9 composes the qualification agent (Classes 4–7) and the research agent (this class) into a real multi-agent workflow — Research → Qualify → Review → Draft → Approve — with an explicit state machine and a human approval gate before any outreach.
2. The `ResearchBrief` this class produces becomes the Qualification Agent's input in Class 9's workflow, replacing the ad hoc account dict it currently reasons over directly.
...
- [ ] Classes 9–11 — see `../../00_Course_Framework.md`
