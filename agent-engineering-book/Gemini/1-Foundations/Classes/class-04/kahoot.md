...
- Question 5 is worth pausing on — it's the first time the class has to locate a bug in prose instructions rather than code, a skill they'll need again in Class 5 when the same procedure moves into a Skill file.
- Question 4 sets up the whole arc from here to Class 6 — write it on the board if useful.
