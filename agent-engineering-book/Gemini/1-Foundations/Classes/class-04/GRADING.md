...
4. **No premature Skill.** If a submission has already created a `skills/` directory or extracted the procedure into a separate file, it's working ahead of the material without the context for why — that's Class 5's job, and doing it here undercuts the "before" state this checkpoint is supposed to demonstrate.
