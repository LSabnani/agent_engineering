...
`EMBEDDED_QUALIFICATION_PROCEDURE` in `qualification_agent.py` is a plain string constant. This is not a mistake to fix — it is the honest "before" state Class 5's Skill extraction is measured against. If you're comparing this checkpoint to a later one and wondering why the procedure "duplicates" what's in `skills/icp_qualification/skill.md` in Class 5, it's because Class 5 is precisely the class that moves it there.

### 4. The agent's prose output format is not yet validated

Nothing in this checkpoint checks that the model's response actually uses one of `QUALIFY` / `DO_NOT_QUALIFY` / `NEEDS_RESEARCH`, or in what format. That gap is closed in Class 6 (structured outputs) — this checkpoint's output is still free-form prose, exactly as intended.
