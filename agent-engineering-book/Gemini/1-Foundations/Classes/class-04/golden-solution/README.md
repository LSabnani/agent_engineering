...
- [ ] The agent has no tools attached — tools arrive in Class 7.
- [ ] No `skills/` directory exists yet — that's Class 5's deliverable, not this one's.
- [ ] `./scripts/check.sh` passes with the live-model tests reporting **skipped**, not failed, when no credentials are set.

## Starting Class 5

1. Start from this checkpoint. Class 5 does not touch `app.py` or the agent's boundary — it extracts the qualification procedure out of `qualification_agent.py`'s embedded string and into `skills/icp_qualification/skill.md`, then updates the agent to load it instead.
2. Everything this checkpoint's tests verify about the agent's construction should still hold after Class 5 — only *where* the procedure text lives changes.
...
- [ ] Classes 5–11 — see `../../00_Course_Framework.md`
