...
- Connecting back: How does this chapter's local playground (§4.6) build on Class 3's "print the assembled context" habit?
...
Adds the ADK agent (embedded procedure), `app.py`, and sample accounts on top of `class-03/`. README notes the Chapter 4 checkpoint explicitly: it can reason about supplied account data but does not yet expose its procedure as a reusable Skill or return a machine-validated contract.
