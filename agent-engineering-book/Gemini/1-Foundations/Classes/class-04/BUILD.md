# Building Class 04 with Antigravity

Goal: a real ADK agent — the Account Qualification Assistant — with its qualification procedure embedded directly in its instruction. `golden-solution/` in this folder is the reference. Build your own copy in `my-work/gemini-book-1/class-04/`, then diff.
...
- Your Class 03 checkpoint, passing `./scripts/check.sh`.
...
4. Write `src/widgetware_sdr/app.py` yourself: a function that constructs the agent, an `InMemorySessionService`, a `Runner`, renders the account (and any notes, delimited exactly like Class 3's evidence section) as the per-call user message, and runs the agent via `runner.run_async(...)`, returning the list of events.
