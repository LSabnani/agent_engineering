...
def test_evidence_coverage_rate_for_qualified_results() -> None:
    qualified_with_evidence = QualificationResult(
        account_id="a", status=QualificationStatus.QUALIFIED, rationale="Fits.", evidence_refs=["ev-1"]
    )
...
def test_evidence_coverage_rate_ignores_non_qualified_results() -> None:
    not_qualified = QualificationResult(
        account_id="b", status=QualificationStatus.NOT_QUALIFIED, rationale="Excluded.", exclusion_reasons=["industry"]
    )
    run = WorkflowRun(account_id="b", qualification_result=not_qualified)
    assert evidence_coverage_rate([run]) == 1.0  # no qualified results to check, defaults to perfect
