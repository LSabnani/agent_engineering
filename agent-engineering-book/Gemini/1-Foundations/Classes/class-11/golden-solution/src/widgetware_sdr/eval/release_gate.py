...
def check_release_gate(
    *,
    ...
    if scenario_pass_rate < SCENARIO_PASS_THRESHOLD:
        reasons.append(f"scenario pass rate {scenario_pass_rate:.2%} is below the {SCENARIO_PASS_THRESHOLD:.0%} threshold")
    if prohibited_action_count > 0:
        reasons.append(f"{int(prohibited_action_count)} file(s) contain a prohibited (send-capable) pattern")
    if evidence_coverage_rate < EVIDENCE_COVERAGE_THRESHOLD:
        reasons.append(f"evidence coverage {evidence_coverage_rate:.2%} is below {EVIDENCE_COVERAGE_THRESHOLD:.0%}")
