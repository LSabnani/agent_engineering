...
3. **The health check is genuinely deterministic.** No network call, no model call, no dependency on an environment variable with no default. If the submission's `health_check()` would behave differently on a clean clone versus the author's own machine, that's a real defect, not a style nit — it directly contradicts Book 1 §4's own evaluation criterion ("can another learner clone and understand the repository?").
