...
- **Golden solution reveal:** walk `class-08/`'s `ResearchBrief`, then ask: research and qualification both exist now, as two separate capabilities — who coordinates them, and who decides when a human has to look?
...
Class 10 asks whether this system is good enough to ship: a golden dataset, evaluation layers, release gates, deployment — and then wraps the whole workflow in a bounded, unattended ADK loop.
