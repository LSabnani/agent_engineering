...
**Manuscript source:** Book 1, Chapter 4 — Your First Agent with ADK
**Seven-Step mapping:** Primary: Build the Harness / Supporting: Design Agent Capabilities, Evaluate & Govern
**Golden solution produced:** `class-03/golden-solution/`
**Starting checkpoint:** `class-02/golden-solution/`

## 0:00–0:30 — Homework review, common mistakes, golden solution reveal

- **Review homework:** ask participants to print their assembled context for a real scenario and read it end to end, per Class 2's own completion checklist.
...
- **Golden solution reveal:** walk `class-02/`'s `context_builder.py`, then say plainly: "today this context finally reaches a real agent."

## Slide outline (0:30–0:55)

1. Current WidgetWare state: real context, no agent yet
2. Today's dependency: Class 2's context package becomes this agent's instructions and per-call message
...
5. Terminology: sessions and events (§4.3), basic state (§4.4)
6. Architecture: the first agent boundary (§4.2) — narrow on purpose, easier to diagnose
...
- Terminology: What four abstractions does ADK give you (§4.1)?
- Terminology: What does an ADK `Session` separate that a raw conversation history doesn't?
- Architecture: Why does Chapter 4 draw such a narrow boundary around the agent's first version?
- Architecture: What's still missing from this agent's output that Chapter 6 will add?
...
- Connecting back: How does this chapter's local playground (§4.6) build on Class 2's "print the assembled context" habit?
...
3. Inspect the event sequence and assembled instructions via the local playground (§4.6).
...
- **Constraints:** no structured/typed output yet (Chapter 6), no Skill yet (Chapter 5), no tools yet (Chapter 7) — the agent's result is still prose, embedded procedure, no external calls, on purpose

## Golden solution: `class-03/`

Adds the ADK agent (embedded procedure), `app.py`, and sample accounts on top of `class-02/`. README notes the Chapter 4 checkpoint explicitly: it can reason about supplied account data but does not yet expose its procedure as a reusable Skill or return a machine-validated contract.

## Bridge to Class 4

Class 4 extracts the qualification procedure out of this agent's embedded instruction string and into a reusable Skill — the agent's boundary and model call don't change, only where its reasoning procedure lives.
