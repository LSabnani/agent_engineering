...
**Manuscript source:** Book 1, Chapter 5 — Gemini Models and Context Engineering
...
4. Core concept: model choice is an architectural decision (§5.1), not an afterthought
5. Terminology: layers of context (§5.2) — instructions, business config, account data, retrieved evidence
6. Architecture: instruction hierarchy (§5.4) and why user content can never override system policy
...
9. Security: prompt-injection-shaped context quality failures (§5.6) — "a malicious note that attempts to override policy"
...
- Terminology: What are the five evidence-policy categories (§5.5)?
- Terminology: What is "context quality failure," and name one kind (§5.6)?
...
- Connecting back: How does the Chapter 3 evidence-vs-inference distinction (Class 1) become enforceable code in this chapter?

## Build together (1:05–1:35)

Following the Hands-on Lab in Book 1 §5:
